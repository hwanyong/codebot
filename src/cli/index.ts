...
/**
 * 작업 디렉토리를 설정합니다.
 * @param workdir 사용자가 지정한 작업 디렉토리 경로 (없을 경우 undefined)
 * @returns 설정된 작업 디렉토리 경로
 *
 * Generated by Copilot
 */
function setupWorkingDirectory(workdir?: string): string {
  // 작업 디렉토리가 지정되지 않은 경우 현재 디렉토리 사용
  if (!workdir) {
    return process.cwd();
  }

  try {
    // 작업 디렉토리 경로 정규화 (절대 경로로 변환)
    const normalizedPath = path.resolve(workdir);

    // 지정된 디렉토리가 존재하는지 확인
    if (!fs.existsSync(normalizedPath)) {
      console.error(chalk.red(i18n.t('workdir_not_found', normalizedPath)));
      return process.cwd(); // 존재하지 않으면 현재 디렉토리 사용
    }

    // 디렉토리인지 확인
    const stats = fs.statSync(normalizedPath);
    if (!stats.isDirectory()) {
      console.error(chalk.red(i18n.t('workdir_not_directory', normalizedPath)));
      return process.cwd(); // 디렉토리가 아니면 현재 디렉토리 사용
    }
    ...
    console.error(chalk.red(i18n.t('workdir_error', error.message)));
    return process.cwd(); // 오류 발생 시 현재 디렉토리 사용
  }
}
...
export function createCLI(): Command {
  ...
  program
    ...
    .version('1.0.0')
    .option('-w, --workdir <directory>', 'Specify working directory. Current directory used if not specified.'); // 작업 디렉토리를 지정합니다. 지정하지 않으면 현재 디렉토리를 사용합니다.

  // 전역 작업 디렉토리 설정
  // Set global working directory
  program.hook('preAction', (thisCommand) => {
    const options = thisCommand.opts();
    const workdir = setupWorkingDirectory(options.workdir);

    // 디버그 모드일 때 작업 디렉토리 변경 정보 출력
    // Display working directory information in debug mode
    if (options.debug) {
      console.log(chalk.gray(`Working directory: ${workdir}`)); // 작업 디렉토리: ${workdir}
    }
  });

  // chat 명령어에 노드별 AI 스트림 제어 옵션 추가
  // Add node-specific AI stream control options to chat command
  program
    .command('chat')
    .description('Start a conversation session with Codebot.') // Codebot과 대화 세션을 시작합니다.
    .option('-m, --model <n>', 'Specify AI model to use.') // 사용할 AI 모델을 지정합니다.
    .option('-p, --provider <n>', 'Specify model provider to use.') // 사용할 모델 제공자를 지정합니다.
    .option('-t, --temperature <number>', 'Set model temperature.', '0.7') // 모델 온도를 지정합니다.
    .option('-v, --verbose', 'Enable detailed logging.') // 상세 로깅을 활성화합니다.
    .option('-d, --debug', 'Enable debug mode.') // 디버그 모드를 활성화합니다.
    .option('-a, --ai-stream', 'Enable AI stream output for all nodes.') // 모든 노드에 대한 AI 스트림 출력을 활성화합니다.
    .option('--stream-nodes <nodes>', 'Comma-separated list of nodes to show AI stream (e.g., translateInput,executeStep)') // 쉼표로 구분된 AI 스트림을 표시할 노드 목록 (예: translateInput,executeStep)
    .option('--hide-stream-nodes <nodes>', 'Comma-separated list of nodes to hide AI stream') // 쉼표로 구분된 AI 스트림을 숨길 노드 목록
  ...
  // Add node-specific AI stream control options to run command
  program
    .command('run')
    .description('Run Codebot with a specific task.') // 특정 작업으로 Codebot을 실행합니다.
    .argument('<task>', 'Task to perform') // 수행할 작업
    .option('-m, --model <n>', 'Specify AI model to use.') // 사용할 AI 모델을 지정합니다.
    .option('-p, --provider <n>', 'Specify model provider to use.') // 사용할 모델 제공자를 지정합니다.
    .option('-t, --temperature <number>', 'Set model temperature.', '0.7') // 모델 온도를 지정합니다.
    .option('-v, --verbose', 'Enable detailed logging.') // 상세 로깅을 활성화합니다.
  ...
  program
    .command('config')
    .description('Manage Codebot settings.') // Codebot 설정을 관리합니다.
    .action(async () => {
      const wizard = new ConfigWizard();
      await wizard.start();
    });
