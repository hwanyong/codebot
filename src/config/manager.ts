...
export class ConfigManager {
  ...
  public getLoggingConfig(): LoggingConfig {
    ...
    if (!this.config.logging) {
      this.config.logging = {
        defaultAiStream: DEFAULT_CONFIG.logging?.defaultAiStream || false,
        alwaysVisibleNodes: DEFAULT_CONFIG.logging?.alwaysVisibleNodes?.slice() || LOGGER_DEFAULTS.ALWAYS_VISIBLE_NODES
      };
    }
  ...
  public updateLoggingConfig(loggingConfig: Partial<LoggingConfig>): void {
    // 기존 설정이 없으면 초기화
    if (!this.config.logging) {
      this.config.logging = DEFAULT_CONFIG.logging || {};
    }
