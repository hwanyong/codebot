...
function routeNode(state: State): string {
  // Early validation to avoid unnecessary checks
  // 불필요한 검사를 피하기 위한 초기 검증
  ...
  // Cases that route to generateResponse
  // generateResponse로 라우팅하는 경우들
  if (
    state.context.directResponse === true || // Direct response requested 직접 응답 요청된 경우
    state.context.verified !== undefined || // Verification completed // 검증이 완료된 경우
    state.context.executionStatus !== 'completed' || // Default case - no other conditions matched 기본 케이스 - 다른 조건이 일치하지 않음
    state.context.requiresToolExecution === false
  ) {
    const reason = state.context.directResponse ? 'Direct response case' :
                   state.context.verified !== undefined ? 'After verification' :
                   'Default route';
    Logger.graphState('Router Decision', `${reason} - route to generateResponse`);
    return 'generateResponse';
  }

  // Execute step route - if there are remaining steps in the execution plan
  // 단계 실행 경로 - 실행 계획에 남은 단계가 있는 경우
  if (
  ...
  // Verification route - execution completed but not yet verified
  // 검증 경로 - 실행은 완료되었지만 아직 검증되지 않음
  if (state.context.executionStatus === 'completed' && state.context.verified === undefined) {
    Logger.graphState('Router Decision', 'Route to verifyResult for verification');
    return 'verifyResult';
  }

  // Final fallback to response generation
  // 응답 생성으로의 최종 폴백
  Logger.graphState('Router Decision', 'Fallback route to generateResponse');
  return 'generateResponse';
}
