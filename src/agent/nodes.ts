...
export async function nodeGenerateResponse(state: State): Promise<Update> {
  ...
  // Check if this is a direct response case
  // 직접 응답 케이스인지 확인
  if (directResponse === true) {
    // Use direct response prompt for simple responses
    // 단순 응답에는 직접 응답 프롬프트 사용
    Logger.nodeAction('generateResponse', 'Creating direct response prompt for simple response');

    const directResponsePrompt = ChatPromptTemplate.fromTemplate(DIRECT_RESPONSE_PROMPT);

    // Render prompt with the task analysis and original request
    // 작업 분석과 원래 요청으로 프롬프트 렌더링
    const promptValue = await directResponsePrompt.formatMessages({
    ...
    try {
      // Call model for direct response
      // 직접 응답을 위한 모델 호출
      Logger.nodeAction('generateResponse', 'Calling model for direct response generation');
      const result = await state.context.model.invoke(promptValue, config);
      Logger.nodeAction('generateResponse', 'Direct response generated successfully');
    ...
    // Render prompt with execution results and verification
    // 실행 결과 및 검증으로 프롬프트 렌더링
    ...
    try {
      // Call model for standard response
      // 표준 응답을 위한 모델 호출
      Logger.nodeAction('generateResponse', 'Calling model for standard response generation');
      const result = await state.context.model.invoke(promptValue, config);
      Logger.nodeAction('generateResponse', 'Standard response generated successfully');
      ...
      Logger.error('Error generating standard response', error);
      Logger.nodeExit('generateResponse', 'error');
      throw error;
    }
  }
