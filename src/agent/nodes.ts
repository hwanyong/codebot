...
export async function nodeTranslateInput(state: State): Promise<Update> {
  ...
  if (isEnglish) {
    ...
    // 실제로는 LangGraph 내부에서 기존 상태와 병합되어 완전한 ContextState가 됩니다.
    return {
      context: {
        ...state.context,
...
export async function nodeExecuteStep(state: State): Promise<Update> {
  ...
  try {
    ...
    if (!tool) {
      throw new Error(`도구를 찾을 수 없습니다: ${toolCall!.tool}`);
    }
